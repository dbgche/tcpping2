...
def conn_tcp(dst_host, dst_port, timeout, src_host=None, src_port=None, rst=False, ipv6=False):
    """
...
def go(dst_host, dst_port, timeout, interval, src_host=None, src_port=None, count=None, src_rotate_port=None, rst=False, ipv6=False
       ):
    error_flag = False
    if src_rotate_port:
    ...
    while judge_count(count):
        (conn_time, close_time, err, local_addr) = conn_tcp(dst_host, dst_port, timeout=timeout, src_host=src_host,
                                                            src_port=src_port, rst=rst, ipv6=ipv6)
...
def getargs():
    ...
    parser.add_argument('-6', '--ipv6', dest='ipv6', action='store_true', help='force using IPv6 instead')
    # 本地IP地址
